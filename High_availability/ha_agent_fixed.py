...
def error_log(error_count,instance_id,old_instance_id=None,e=None,ha_agent=None):
    if(error_step==1):
        ha_agent.error("< %s > Unknown Exception : "%instance_id,e)
    elif(error_step==2):
        ha_agent.error("< %s > Dump Directory Creation failed with Exception :"%instance_id,e)
    elif(error_step==3):
        ha_agent.error("< %s > Client Initialization failed with Exception :"%instance_id,e)
    elif(error_step==4):
        ha_agent.error("< %s > Info collection failed with Exception :"%instance_id,e)
    elif(error_step==5):
        ha_agent.error("< %s > Json Dump write of old Instance failed with Exception :"%instance_id,e)
    elif(error_step==6):
        ha_agent.error("< %s > DoT status update failed with Exception :"%instance_id,e)
    elif(error_step==7):
        ha_agent.error("< %s > Instance Deletion failed with Exception :"%instance_id,e)
    elif(error_step==8):
        ha_agent.error("< %s > BDM reset failed with Exception :"%instance_id,e)
    elif(error_step==9):
        ha_agent.error("< %s > New Instance Creation failed with Exception :"%old_instance_id,e)
    elif(error_step==10):
        ha_agent.error("< %s > Info collection on New Instance: < %s > failed with Exception :"%(old_instance_id,instance_id),e)
    elif(error_step==11):
        ha_agent.error("< %s > DoT status update on New Instance: < %s > failed with Exception :"%(old_instance_id,instance_id),e)
    elif(error_step==12):
        ha_agent.error("< %s > Floationg IP attach on New Instance: < %s >  failed with Exception :"%(old_instance_id,instance_id),e)
    elif(error_step==13):
        ha_agent.error("< %s > Volume attach on New Instance: < %s > failed with Exception :"%(old_instance_id,instance_id),e)
    elif(error_step==13):
        ha_agent.error("< %s > Json Dump creation of New Instance: < %s > failed with Exception :"%(old_instance_id,instance_id),e)
    else:
...
def migrate(instance_id,time_suffix,remigration=None,host_name=None,ha_agent=ha_agent):
    try:
        ...
        nics = get_fixed_ip(info,neutron) # Fixed IPs
        ...
        old_json,old_json_encoded = update_step_count(data=old_json,step_count=error_step)
        ...
        try:
            ...
            for flt_ip , fix_ip in ip_list:
                remove_floating_ip(nova,old_instance_id,flt_ip)

            for nic in nics:
                if nic.has_key('v4-fixed-ip'):
                    remove_fixed_ip(nova,old_instance_id,nic.get('v4-fixed-ip'))
        ...
        new_instance_json,Encoded_new_instance_json=json_dump_edit(data=old_json,new_instance_id=new_instance_id,\
                                                                   new_host_name=new_host_name,step_count=error_step)
        ...
        if remigration and host_name:            
            if any(issubclass(e.__class__, lv) for lv in kazoo_exceptions):
                ...
                if(error_step<9):
                    ...
                    error_log(error_step,old_instance_id,e=e,ha_agent=ha_agent)
                    ...
                    error_log(error_step,new_instance_id,e=e,ha_agent=ha_agent,old_instance_id=old_instance_id)
